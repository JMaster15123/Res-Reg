...
from resnet_regressor import resnet_regressor
from loss import *
...
os.makedirs(os.path.join(args.store_root, args.store_name), exist_ok=True)
...
def main():
    ...
    df_train, df_val, df_test = df[df['split'] == './o_train'], df[df['split'] == './o_val'], df[df['split'] == './o_test']
    ...
    model = resnet_regressor(fds=args.fds, bucket_num=args.bucket_num, bucket_start=args.bucket_start,
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        val_loss_mse, val_loss_l1, val_loss_gmean, val_preds, val_labels = validate(val_loader, model,
        ...
        val_mse_all = []
        val_gmean_all = []
        val_l1_all.append(val_loss_l1)
        val_mse_all.append(val_loss_mse)
        val_gmean_all.append(val_loss_gmean)
    ...
    test_loss_mse, test_loss_l1, test_loss_gmean, test_preds, test_labels = validate(test_loader, model,
                                                                                     train_labels=train_labels,
                                                                                     prefix='Test')
...
def shot_metrics(preds, labels, train_labels, many_shot_thr=2000, low_shot_thr=20):
...
def validate(val_loader, model, train_labels=None, prefix='Val'):
    ...
    return losses_mse.avg, losses_l1.avg, loss_gmean, preds, labels
